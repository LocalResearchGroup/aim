...
def get():
    ...
    return Table(Tr(Th('Run ID'), Th('Upload IDs'), Th('In Upload'), Th('In Processed')),
        *[Tr(Td(run_id), Td(*[A(u, hx_post=f'/reprocess_upload?upload_id={u}', hx_swap='outerHTML') if not v['in_processed'] else Div(u) for u in v['upload_ids']]), Td('✅' if v['in_upload'] else '❌'), Td('✅' if v['in_processed'] else '❌')) for run_id, v in compared_run_ids.items()],id='run_status_table')
